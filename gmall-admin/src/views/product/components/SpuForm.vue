<template>
  ...
  <el-form :model="spuForm" label-width="70px">
    ...
    <div>
      <el-table
        v-loading="pmsProductSaleAttrListLoading"
        ref="spuSaleAttrTable"
        :data="pmsProductSaleAttrListTemp"
        ...
        <el-table-column label="销售属性值">
          <template slot-scope="scope">
            <el-tag
              v-for="tag in scope.row.pmsProductSaleAttrValueList"
              :key="tag"
...
export default {
  ...
  data() {
    return {
      ...
      pmsProductSaleAttrListTemp: [], // 临时数据：格式：saleAttrId|saleAttrName

      // 销售属性列表loading
      pmsProductSaleAttrListLoading: false,
  ...
  methods: {
    ...
    saveSpuInfo() {
      ...
      this.spuForm.pmsProductSaleAttrList = []
      this.pmsProductSaleAttrListTemp.forEach(spuSaleAttr => {
        // 处理销售属性
        ...
        // 处理销售属性值
        spuSaleAttr.pmsProductSaleAttrValueList.forEach(spuSaleAttrValue => {
          const saleAttrValue = {
            saleAttrId: arr[0],
            saleAttrValueName: spuSaleAttrValue
          }
          saleAttr.pmsProductSaleAttrValueList.push(saleAttrValue)
        })

        this.spuForm.pmsProductSaleAttrList.push(saleAttr)
      })
    ...
    onUploadSuccess(res, file) {
      // 填充上传文件列表
      this.spuForm.pmsProductImageList.push({
        imgName: file.name,
    ...
    onUploadRemove(file, fileList) {
      this.spuForm.pmsProductImageList = []
    ...
    addSaleAttr() {
      const saleAttr = {
        ...
        edit: false,
        pmsProductSaleAttrValueList: [] // [tag1, tag2, tag3]
      }
      this.pmsProductSaleAttrListTemp.push(saleAttr)
    },

    // 根据name删除销售属性
    deleteSaleAttr(saleAttr) {
      const tempList = []
      this.spuForm.pmsProductSaleAttrList.forEach(item => {
        if (item.saleAttr !== saleAttr) {
          tempList.push(item)
        }
      })
      this.spuForm.pmsProductSaleAttrList = tempList
    },
    ...
    // 保存销售属性值
    saveAttrValue(row) {
      if (!row.pmsProductSaleAttrValueList) {
        row.pmsProductSaleAttrValueList = []
      }

      const saleAttrValue = row.saleAttrValue
      if (saleAttrValue) {
        row.pmsProductSaleAttrValueList.push(saleAttrValue)
      }
    ...
    handleTagClose(tag, row) {
      const index = row.pmsProductSaleAttrValueList.indexOf(tag)
      row.pmsProductSaleAttrValueList.splice(index, 1)
    }
  }
}
</script>
